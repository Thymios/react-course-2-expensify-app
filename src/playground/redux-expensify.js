...
const getVisibleExpenses = (expenses, { text, sortBy, startDate, endDate }) => {
    return expenses.filter((expense) => {
        const startDateaMatch = typeof startDate !== 'number' || expense.createdAt >= startDate;
        const endDateMatch = typeof endDate !== 'number' || expense.createdAt <= endDate;
        const textMatch = expense.description.toLowerCase().includes(text.toLowerCase());

        return startDateaMatch && endDateMatch && textMatch;
    }).sort((a, b) => {
        if (sortBy === 'date') {
...
// store.dispatch(setStarDate(-1000));
...
const demoState = {
    expenses: [{
        ...
        nte: 'This was the final payment for that address',
        amount: 54500,
        createdAt: 0
    }],
    filters: {
